...
export class ObservabilityService {
    ...
    for (const { subgroup, desired } of grafanaRbacMembershipMappings(listPerms)) {
      ...
      for (const userId of desired) {
        if (!group.members.some(m => m.id === userId)) {
          promises.push(this.addUserToGroupSafe(userId, group.id))
        }
      }

      for (const member of group.members) {
        if (!desiredSet.has(member.id)) {
          promises.push(this.removeUserFromGroupSafe(member.id, group.id))
        }
      }
    }
  ...
  private async addUserToGroupSafe(userId: string, groupId: string): Promise<void> {
    try {
  ...
  private async removeUserFromGroupSafe(userId: string, groupId: string): Promise<void> {
    try {
