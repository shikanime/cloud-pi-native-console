...
describe('sonarqubeService', () => {
  ...
  beforeEach(async () => {
    ...
    gitlab = mockDeep<GitlabClientService>({
      setGitlabGroupVariable: vi.fn().mockResolvedValue('created'),
      setGitlabRepoVariable: vi.fn().mockResolvedValue('created'),
      getOrCreateProjectGroup: vi.fn().mockResolvedValue({ id: 1, full_path: 'root', name: 'root' }),
      getOrCreateProjectGroupRepo: vi.fn().mockResolvedValue({ id: 2 }),
    })
  ...
  describe('handleUpsert', () => {
    ...
    it('should provision GitLab CI variables for each repository and the shared group token', async () => {
      ...
      expect(gitlab.setGitlabRepoVariable).toHaveBeenCalledWith(expect.any(Number), 'PROJECT_KEY', key, expect.objectContaining({ masked: false, variableType: 'env_var', environmentScope: '*' }))
      expect(gitlab.setGitlabRepoVariable).toHaveBeenCalledWith(expect.any(Number), 'PROJECT_NAME', `${project.slug}-repo`, expect.objectContaining({ masked: false, variableType: 'env_var', environmentScope: '*' }))
      expect(gitlab.setGitlabRepoVariable).toHaveBeenCalledWith(expect.any(Number), 'SONAR_PROJECT_PROPERTIES', expect.stringContaining(`sonar.projectKey=${key}`), expect.objectContaining({ masked: false, variableType: 'file', environmentScope: '*' }))
      expect(gitlab.setGitlabGroupVariable).toHaveBeenCalledWith(expect.any(Number), 'SONAR_TOKEN', 'tok', expect.objectContaining({ masked: true, variableType: 'env_var' }))
    })
